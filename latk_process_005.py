...
def main():
    ...
    for i in range(0, len(urls)):        
        ...
        for j, label in enumerate(kmeans.labels_):
            ...
            clusters[label].colors.append(mesh.visual.vertex_colors[j])
        ...
        for j, cluster in enumerate(clusters):
            ...
            clusterMesh1 = trimesh.Trimesh(vertices=cluster.points)
            clusterMesh1.export(newTempUrl)
            ms = ml.MeshSet()
            ms.load_new_mesh(newTempUrl) # first mesh -> index 0
            clusterMesh2 = ms.current_mesh()

            ms.add_mesh(clusterMesh2) # duplicates the current mesh -> index 1
            ms.surface_reconstruction_ball_pivoting()
            ...
            clusterMesh3 = trimesh.load(newTempUrl)
            edges = clusterMesh3.edges_unique
            length = clusterMesh3.edges_unique_length
            ...
            while strokeCounter < int(numStrokes / len(clusters)):
                ...
                try:
                    ...
                    for index in path:
                        vert = cluster.points[index] #vertices[index]
                        similarity.append(str(index))

                        col = cluster.colors[index]
                ...
                if (len(points) >= minStrokePoints):  
                    ...
                    if (readyToAdd):
                        ...
                        print(sayText + "created stroke " + str(strokeCounter) + " / " + str(numStrokes) + ", with " + str(len(points)) + " points")
                else:
