...
python vox2vox/train.py --epoch "$START_EPOCH" --dataset "input128" --img_width "$DIMS" --img_height "$DIMS" --img_depth "$DIMS" --batch_size "$BATCH_SIZE" --n_cpu "$NUM_CPUS" --checkpoint_interval "$CHECKPOINT_INTERVAL" --n_epochs "$TOTAL_EPOCHS"
